...
#### Класс Modal
Реализует модальное окно. Предоставляет методы для открытия, закрытия и рендера содержимого.\
Устанавливает обработчики для закрытия по клику на кнопку-крестик и на оверлей, а также предотвращает закрытие при клике на содержимое.

- constructor(container: HTMLElement, events: IEvents) — конструктор принимает контейнер модального окна и экземпляр брокера событий. В конструкторе находятся элементы кнопки закрытия и контейнера содержимого, а также устанавливаются обработчики событий.

Поля класса:
- container: HTMLElement — элемент контейнера модального окна.

- events: IEvents — брокер событий, экземпляр EventEmitter.
...
Методы класса:
- set content(value: HTMLElement): void — заменяет содержимое модального окна переданным элементом.

- open(): void — открывает модальное окно, добавляя CSS-класс modal_active, и вызывает событие modal:open.

- close(): void — закрывает модальное окно, удаляет содержимое и вызывает событие modal:close.

- render(data: IModalData): HTMLElement — рендерит переданное содержимое, открывает окно и возвращает контейнер.
...
- basketTemplate: HTMLTemplateElement — шаблон карточки для корзины.

- events: EventEmitter — брокер событий для инициации пользовательских событий.

- categoryColors: Record<string, string> — объект, связывающий названия категорий с CSS-модификаторами для стилизации.

Так же класс предоставляет набор методов для взаимодействия с этими данными:

- fillCardBasicFields(clone: DocumentFragment, product: IProduct, selector: string = '.card') — заполняет элементы шаблона данными продукта (заголовок, изображение, категория, цена) и применяет стиль категории.

- createCardFromTemplate(template: HTMLTemplateElement, product: IProduct, mainSelector: string) — создает карточку из переданного шаблона и заполняет её данными через fillCardBasicFields.

- private applyCategoryStyle(categoryElement: HTMLElement, categoryName: string): void — применяет соответствующий цветовой стиль к элементу категории на основе её названия, удаляя старые классы и добавляя новый.
...
#### Класс MainPageView
Отвечает за отображение главной страницы: галереи товаров, кнопки корзины и счётчика товаров.\
Также реагирует на открытие и закрытие модальных окон.
...
- handleModalOpen(): void — при открытии модального окна добавляет класс modal_no-scroll к body, чтобы запретить прокрутку страницы.

- handleModalClose(): void — при закрытии модального окна удаляет класс modal_no-scroll.

- render(cards: HTMLElement[]): void — отображает карточки товаров в галерее.

- setCounter(count: number): void — устанавливает число товаров в корзине в счётчик.
